...
function renderBatch(results) {
  ...
  const pdfRows = results.map((result) => { const row = result.row || {}; const orgStatus = result.orgMatch === true ? "일치" : result.orgMatch === false ? "불일치" : "확인 필요"; const dateStatus = result.dateMatch === true ? "일치" : result.dateMatch === false ? "불일치" : "확인 필요"; return `<tr><td>${escapeHtml(row.name || result.record.name || "-")}<small class="table-file">${escapeHtml(result.record.fileName)}</small></td><td>${escapeHtml(row.organization || "-")}</td><td>${escapeHtml(result.record.organization || "-")}</td><td class="status-${result.orgMatch ? "ok" : "check"}">${orgStatus}</td><td>${escapeHtml(row.position || "-")}</td><td>${escapeHtml(row.date || "-")}</td><td>${escapeHtml(result.record.acquiredDate || "-")}</td><td class="status-${result.dateMatch ? "ok" : "check"}">${dateStatus}</td><td>${escapeHtml(row.duty || "-")}</td><td>${escapeHtml(row.employmentType || "-")}</td><td class="status-${result.status === "일치" ? "ok" : "check"}">${escapeHtml(result.status)}</td></tr>`; }).join("");
